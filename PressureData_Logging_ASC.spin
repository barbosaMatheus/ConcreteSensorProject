...
pub main | cog, pressure1, pressure2
  ...
  repeat 
    ...
    pressure1 := getPressure(cds1Pin)
    if(pressure1 > 20 or pressure1 < -20)
      sd.popen(string("data.docx"), "a")              ''open file to append
      sd.pputs(exp.toStr(rtc.getMonths))              ''date
      sd.pputc("/")
      sd.pputs(exp.toStr(rtc.getDOW))
      sd.pputc(" ")
      ...
      sd.pputs(exp.toStr(pressure1))       ''pressure 1
      sd.pputs(string("Pa "))
...
pri getPressure(pin)      ''converts adc reading to pressure
  ...
  return((((adc.read(pin)-initialADReading)*13)/10))
